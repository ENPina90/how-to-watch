...
module EntryCardHelper
  ...
  # cable schedule lays a slot out by this figure and keeps an entry off the air where it is
  # missing -- which is usually why somebody is here. A show has no runtime of its own, so
  # for a series the field is not read by cable at all and the episodes are where the real
  # answer belongs.
  def runtime_hint_for(entry)
    if entry.media.in?(%w[series anime])
      'A show has no runtime of its own -- set it per episode below. Cable only airs the ' \
        'episodes that have one.'
    else
      'Used by the cable schedule. Left blank, it is kept off cable.'
    end
  end
