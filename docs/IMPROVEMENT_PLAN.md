...
**Done 2026-08-25 (Devise 5.0.4).** The deprecations are gone. The risk that mattered was
stored password digests: a digest generated by 4.9.4 was captured *before* the upgrade and
confirmed to still authenticate after it. `spec/requests/authentication_spec.rb` now drives
...
- ~~**Gemfile:** `simple_form` pinned to a git branch, `webdrivers` abandoned…~~
  **Done 2026-08-25**: `simple_form` pinned to `~> 5.4` (a git ref meant the build depended
  on whatever `main` pointed at that day), `webdrivers` removed (Selenium 4.6+ resolves
  drivers itself, and there are no system tests). Everything else was brought current in
  the same pass — Rails 8.1.3.1, Devise 5, Sidekiq 8, Puma 8, redis 6, importmap-rails 2,
  turbo-rails 2, rspec-rails 8, dotenv 3. `bundle outdated --only-explicit` is clean.
- ~~**Empty CSP initializer**~~ **Done 2026-08-25.**
  - **`Permissions-Policy` is enforced**, denying 17 hardware/credential features the app
    never uses (camera, microphone, geolocation, payment, USB, serial, HID, sensors…) and
    explicitly allowing the four the player needs (`autoplay`, `fullscreen`,
    `encrypted-media`, `picture-in-picture`). This is the part that constrains the embedded
    provider: an iframe can only use a feature the embedding page delegates.
    Set as a default header rather than via Rails' `permissions_policy` DSL, because that
    DSL still emits the **superseded `Feature-Policy`** header with the old
