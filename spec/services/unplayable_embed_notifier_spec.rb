...
RSpec.describe UnplayableEmbedNotifier do
  ...
  def audit_returning(rows, checked: 1)
    instance_double(EmbedAvailabilityAudit,
                    call: EmbedAvailabilityAudit::Result.new(checked: checked, suspected: rows.size,
                                                             missing: rows, unknown: [], skipped: 0))
  end
  ...
  it 'raises one notification per unplayable entry, pointed at it' do
    described_class.new(audit: audit_returning([row])).call
  ...
  it 'retires the notification once VidSrc has the title' do
    ...
    expect { described_class.new(audit: audit_returning([])).call }
