...
RSpec.describe 'The admin dashboard', type: :request do
  ...
  describe 'the maintenance sweeps' do
    ...
    it 'enqueues the new episode check' do
      ...
      expect { post run_episode_scan_admin_dashboard_path }
        .to have_enqueued_job(NewEpisodeScanJob)
      expect(response).to redirect_to(admin_dashboard_path)
    end

    it 'does not let a member start the episode check' do
      sign_in create(:user)

      expect { post run_episode_scan_admin_dashboard_path }.not_to have_enqueued_job(NewEpisodeScanJob)
    end
